...
const Faculdade = () => {
    ...
    const entityService = new FaculdadeService();
    ...
    const saveEntity = () => {
        ...
        if (entity.nome.trim()) {
            // let _entities = [...entityList];
            let _entity = {...entity};
            if (entity.id) {
                // const index = findIndexById(entity.id);
                entityService.save(_entity);
                //_entities[index] = _entity;
                toast.current.show({ severity: 'success', summary: 'Sucesso', detail: 'Entidade Atualizada', life: 3000 });
            }
            else {
                _entity.image = 'product-placeholder.svg';
                entityService.add(_entity);
                //_entities.push(_entity);
                toast.current.show({ severity: 'success', summary: 'Sucesso', detail: 'Entidade Criada', life: 3000 });
            }
            entityService.getAll().then(data => setEntityList(data));
    ...
    const deleteEntity = () => {
        ...
        setDeleteEntityDialog(false);
        setEntity(emptyEntity);
        toast.current.show({ severity: 'success', summary: 'Sucesso', detail: 'Entidade Apagada', life: 3000 });
        entityService.getAll().then(data => setEntityList(data));
    }
    ...
    const deleteSelectedEntities = () => {
        ...
        toast.current.show({ severity: 'success', summary: 'Sucesso', detail: 'Entidades Apagadas', life: 3000 });
        entityService.getAll().then(data => setEntityList(data));
    }
        ...
        <div className="datatable-crud-demo">
            ...
            <div className="card">
                ...
                <DataTable ref={dt} value={entityList} selection={selectedEntities} onSelectionChange={(e) => setSelectedEntitites(e.value)}
                    ...
                    <Column field="classificacao" header="Classificação" body={ratingBodyTemplate} sortable></Column>
                ...
                <div className="p-field">
                    ...
                    <div className="p-formgrid p-grid">
                        <div className="p-field-radiobutton p-col-6">
                            <RadioButton inputId="tipo1" name="tipo" value="publica" onChange={(e) => {onSelectChange(e, 'tipo')}} checked={entity.category === 'publica'} />
                            <label htmlFor="tipo1">Pública</label>
                        </div>
                        <div className="p-field-radiobutton p-col-6">
                            <RadioButton inputId="tipo2" name="tipo" value="privada" onChange={(e) => {onSelectChange(e, 'tipo')}} checked={entity.category === 'privada'} />
                            <label htmlFor="tipo2">Privada</label>
                        </div>
                    </div>
                </div>
            </Dialog>
